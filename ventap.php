...
foreach ($ventas_list as $value) {
	...
	foreach ($v as $key) {
		...
		$datos .='<tr>
    			...
    			<td class="precio_venta text-end">'.$value['precio_venta'].'</td>
    			<td class="text-center"><button id="agregar" value="'.$value['id_producto'].'" class="btn-action btn-add"><i class="fas fa-plus"></i></button></td>
    		  </tr>';
	}
    $i++;
}
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/limonte-sweetalert2/11.7.32/sweetalert2.min.css">
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-control, .form-select { border: 2px solid #e0e0e0; border-radius: 10px; padding: 12px 15px; transition: all 0.3s ease; }
        ...
        .total-box { background: linear-gradient(135deg, #f093fb, #f5576c); color: white; padding: 20px; border-radius: 10px; text-align: right; margin: 20px 0; }
        .total-box h4 { margin: 0; font-size: 1.5rem; }
        .items-table { margin-top: 15px; }
        .items-table input { border: 1px solid #e0e0e0; border-radius: 5px; padding: 8px; width: 80px; text-align: center; }
        .items-table input.mon { width: 100px; background: #f8f9fa; }
        .items-table .borrar { background: linear-gradient(135deg, #ff416c, #ff4b2b); color: white; border: none; border-radius: 5px; width: 30px; height: 30px; cursor: pointer; transition: all 0.3s ease; }
        ...
        .proforma-badge { background: linear-gradient(135deg, #f093fb, #f5576c); color: white; padding: 5px 15px; border-radius: 20px; font-size: 0.85rem; }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar">
            <h1><i class="fas fa-file-alt me-2"></i>Proforma</h1>
            <div class="user-info">
        ...
        <div class="content-container">
            <div class="main-panel">
                <div class="sub-nav">
                    ...
                    <a href="ventap.php" class="nav-btn active"><i class="fas fa-file-alt me-2"></i>Proforma</a>
                    <a href="venta_comprobantes.php" class="nav-btn"><i class="fas fa-receipt me-2"></i>Comprobantes</a>
                </div>

                <div class="content-card">
                    <div class="card-header-custom">
                        <h5><i class="fas fa-file-alt me-2"></i>Nueva Proforma</h5>
                        <span class="proforma-badge"><i class="fas fa-file-invoice me-1"></i>Cotizacion</span>
                    </div>
                    <form id="venta">
                        <div class="row mb-4">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <label class="form-label"><i class="fas fa-user me-2"></i>Cliente</label>
                        ...
                        <div class="card-header-custom">
                            <h5><i class="fas fa-list me-2"></i>Items de la Proforma</h5>
                        </div>

                        <div class="table-responsive items-table">
                            <table id="items" class="table modern-table">
                                <thead>
                                    <tr>
                                        <th>Cantidad</th>
                                        <th>Descripcion</th>
                        ...
                        <div class="total-box">
                            <h4><i class="fas fa-calculator me-2"></i>TOTAL: S/ <input id="total" name="total" type="text" value="0.00" style="background:transparent; border:none; color:white; font-size:1.5rem; width:120px; text-align:right; font-weight:bold;"></h4>
                        </div>

                        <button type="button" id="guardar_venta" class="btn btn-submit">
                            <i class="fas fa-save me-2"></i>Generar Proforma
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="side-panel">
                <div class="content-card">
                    <div class="card-header-custom">
                        <h5><i class="fas fa-box me-2"></i>Agregar Productos</h5>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#datos').on('click', '#agregar', function(){
                ...
                $('#items tr:last').after('<tr class="child"><input type="hidden" value="'+id_p+'" name="id_pro[]"><td><input class="cantidad" type="number" value="'+cantidad+'" name="cantidad[]" min="1"></td><td style="text-transform:uppercase;">'+nombre+'</td><td style="text-transform:uppercase;">'+unidad+'</td><td><input name="fv[]" type="text" class="fv" value="'+fv+'"></td><td><input type="number" class="pre" value="'+precio+'" name="precio[]" step="0.01"></td><td><input class="mon" type="text" value="'+monto+'" name="total_pre[]" readonly></td><td><button type="button" value="'+monto+'" class="borrar"><i class="fas fa-times"></i></button></td></tr>');
            ...
            $('body').on('change paste keyup', ".pre", function(){
                ...
                $(this).closest('tr').find('.mon').val(monto1.toFixed(2));
                $(this).closest('tr').find('.borrar').val(monto1.toFixed(2));
            });
                ...
                $.ajax({
                    ...
                    success: function(response){
                        if(response.respuesta == false){
                            ...
                            Swal.fire('Perfecto', 'Proforma generada correctamente', 'success');
