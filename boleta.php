...
<div id="overlay-facturando" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.82);z-index:9999;align-items:center;justify-content:center;flex-direction:column;">
	<div style="text-align:center;color:#fff;">
		...
		<h3 style="margin:0 0 10px;">Generando boleta...</h3>
...
<!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
	...
	$(document ).ready(function() {
		...
		window.onbeforeunload = function() {
			if (procesando) return 'La boleta está siendo generada. ¿Está seguro que desea salir?';
		};
...
//borrar item
		    ...
		    $.ajax({
				...
				success: function(response){
					...
					if (sunat.ya_existia) {
						$btn.text('✓ Boleta ya existía');
						alert('Esta venta ya tiene una boleta generada. Se abrirá el documento existente.');
					} else {
				...
				error: function(){
					...
					alert('Error al generar boleta. Intente nuevamente.');
				}
			});
		});
