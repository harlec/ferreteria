        ...
        <div class="content-container">
            <div class="main-panel">
                <div class="sub-nav">
                    ...
                    <a href="categorias.php" class="nav-btn active"><i class="fas fa-tags me-2"></i>Categorías</a>
                ...
                <div class="content-card">
                    <div class="card-header-custom">
                        <h5><i class="fas fa-tags me-2"></i>Lista de Categorías</h5>
                        <span class="badge bg-primary"><?php echo count($categorias_list); ?> categorías</span>
                    </div>
                    <div class="table-responsive">
                        <table id="datos" class="table modern-table">
                            <thead><tr><th>ID</th><th>Categoría</th><th>Acciones</th></tr></thead>
                            <tbody>
                                <?php foreach ($categorias_list as $value) {
                                    echo '<tr>
                                        <td><strong>#' . $value['id_categoria'] . '</strong></td>
                                        <td id="' . $value['id_categoria'] . '" class="nom_cat">' . htmlspecialchars($value['nom_cat']) . '</td>
                                        <td><button class="btn-action btn-edit editar_c" value="' . $value['id_categoria'] . '" title="Editar"><i class="fas fa-edit"></i></button></td>
                                    </tr>';
                                } ?>
                            </tbody>
                        </table>
            ...
            <div class="side-panel">
                <div class="content-card">
                    <div class="card-header-custom">
                        <h5 id="tituc"><i class="fas fa-plus-circle me-2"></i>Nueva Categoría</h5>
                    </div>
                    <form>
                        <div class="mb-4">
                            <label class="form-label">Nombre de la Categoría</label>
                            <input class="form-control" type="text" name="categoria" id="categoria" placeholder="Ej: Herramientas">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', "#guardar_cate", function() {
                ...
                $.ajax({
                    type: "POST", dataType: 'json', url: '/inc/registrar_categoria.php', data: str1,
            ...
            $('body').on('click', ".editar_c", function() {
                ...
                $('#tituc').html('<i class="fas fa-edit me-2"></i>Editar Categoría');
                $('#categoria').val(nombre);
                $('#guardar_cate').html('<i class="fas fa-save me-2"></i>Editar').val('editar');
                $('#id').val(id);
            });

            function resetForm() { $('#tituc').html('<i class="fas fa-plus-circle me-2"></i>Nueva Categoría'); $('#guardar_cate').html('<i class="fas fa-save me-2"></i>Guardar').val('nuevo'); $('#categoria').val(''); $('#id').val(''); }
