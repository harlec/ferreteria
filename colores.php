...
$colores = Sdba::table('color');
$colores_list = $colores->get();
?>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .color-preview { width: 25px; height: 25px; border-radius: 50%; display: inline-block; vertical-align: middle; margin-right: 10px; border: 2px solid #e0e0e0; }
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar">
            <h1><i class="fas fa-palette me-2"></i>Colores</h1>
            <div class="user-info">
        ...
        <div class="content-container">
            <div class="main-panel">
                <div class="sub-nav">
                    ...
                    <a href="categorias.php" class="nav-btn"><i class="fas fa-tags me-2"></i>Categorias</a>
                    <a href="marcas.php" class="nav-btn"><i class="fas fa-industry me-2"></i>Marcas</a>
                    <a href="colores.php" class="nav-btn active"><i class="fas fa-palette me-2"></i>Colores</a>
                </div>
                <div class="content-card">
                    <div class="card-header-custom">
                        <h5><i class="fas fa-palette me-2"></i>Lista de Colores</h5>
                        <span class="badge bg-primary"><?php echo count($colores_list); ?> colores</span>
                    </div>
                    <div class="table-responsive">
                        <table id="datos" class="table modern-table">
                            <thead><tr><th>ID</th><th>Color</th><th>Acciones</th></tr></thead>
                            <tbody>
                                <?php foreach ($colores_list as $value) {
                                    echo '<tr>
                                        <td><strong>#' . $value['id_color'] . '</strong></td>
                                        <td id="' . $value['id_color'] . '" class="nom_cat">' . htmlspecialchars($value['color']) . '</td>
                                        <td><button class="btn-action btn-edit editar_c" value="' . $value['id_color'] . '" title="Editar"><i class="fas fa-edit"></i></button></td>
                                    </tr>';
                                } ?>
                            </tbody>
                        </table>
            ...
            <div class="side-panel">
                <div class="content-card">
                    <div class="card-header-custom">
                        <h5 id="tituc"><i class="fas fa-plus-circle me-2"></i>Nuevo Color</h5>
                    </div>
                    <form>
                        <div class="mb-4">
                            <label class="form-label">Nombre del Color</label>
                            <input class="form-control" type="text" name="categoria" id="categoria" placeholder="Ej: Rojo, Azul, Verde">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', "#guardar_cate", function() {
                ...
                $.ajax({
                    type: "POST", dataType: 'json', url: '/inc/registrar_color.php', data: str1,
            ...
            $('body').on('click', ".editar_c", function() {
                ...
                $('#tituc').html('<i class="fas fa-edit me-2"></i>Editar Color');
                $('#categoria').val(nombre);
                $('#guardar_cate').html('<i class="fas fa-save me-2"></i>Editar').val('editar');
                $('#id').val(id);
            });

            function resetForm() { $('#tituc').html('<i class="fas fa-plus-circle me-2"></i>Nuevo Color'); $('#guardar_cate').html('<i class="fas fa-save me-2"></i>Guardar').val('nuevo'); $('#categoria').val(''); $('#id').val(''); }
